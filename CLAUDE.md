...
| 6     | Two-hand transform core                                              | ✅ done (D50) — awaiting user's real-hand check                           | `phase-6` commit     |
| 7     | Spatial Panel + Texture Surface                                      | ⏭ **next** (order after Phase 6: 7 → 9…, D46)                             | —                    |
...
**main-user lock** (keeps one person's pair — see D23; MediaPipe asked for 2 hands, D41), stable left/right
...
strokes stay aligned through resizes (D47). **Phase 6 — two-hand transform** (D50): one shared
controller (`modes/shared/TwoHandTransform.ts`) holds things with both hands — no jump when the
grab starts, each pinch stays on the same spot of the object, one undo step per grab ("Move
...
5. **Phase 5 (Voxel Builder) is DONE, but the user's real-hand check is STILL PENDING** — on
   2026-09-26 the user said "we will get back to this" and moved on to Phase 8 (D46). When they
   return to it, ask for the check: stationary pinch =
...
7. **Phase 6 (two-hand transform, D50) is DONE — ask for the user's real-hand check** in the Voxel
   Builder: both hands pinch → move / turn / resize the structure; no jump when it starts; Ctrl+Z
   undoes a whole grab; Reset view (R) then Ctrl+Z brings the view back; drop one hand out of view
   mid-grab (status "Hand lost — the structure holds still…"). Does anything feel slowed down
...
8. **Order from here (D46): Phase 7 next, then 9 → 10…** Phase 7 is **Spatial Panel + Texture
   ...
   margin; call `begin()` only when the pinch is on it). Stop and report.
...
### Phase 6 — Two-hand transform core — spec §12 ✅ DONE

Built — see §9 and D50. Accept checks in `tests/unit/twoHandTransform.test.ts` (no jump, the held
spots stay under the pinches, freeze / re-anchor / release on hand loss, ±180° turns + a rename
mid-grab, crossing close together, glitch limit, size limits, width-only, undo / mode switch
mid-grab) and `tests/integration/voxelBuilder.test.ts` (synthetic stretch + the user's crossing
recording). Plan kept for reference:
...
- **2026-09-25 — Session 2 (second laptop).** Set up the user's second laptop: Intel UHD drives a
  ...
  against Phase 2 in their Chrome, which found the root cause, then capped drawing (D40). The
...
- **2026-09-26 — Session 2 (cont.). Air Draw: point to draw (D48).** User tried pinch drawing:
  "not very smooth"; asked to draw with the pointing index fingertip instead. Their screenshot
  also showed the steady-aim cursor ~30 px off the fingertip. Measured on 212 real motion windows
  which pen source is smoothest: sampling only real tracker readings (not predicted frames) halves
  the drawn line's shape error; the curve type made no difference. Built it; `ModeRig.point()` and
...
- **2026-09-27 — Session 2 (cont.). Phase 6 — two-hand transform built.** The user said "lets start
  with phase 6". Replayed their crossing recording (9 two-hand grabs) first: a grab begun with the
  hands 0.05 apart asked the structure for 10.6×, and hands passing each other turned it 161°. Built
  `TwoHandTransform` (D50) with a spread floor, a turn fade for close hands, per-second safety
  limits set above their fastest real grab (the first guess, 4 / 4 / 8, visibly held back 5 of the 9
  grabs), freeze + re-anchor on hand loss, one undo step per grab; Reset view became undoable.
  Voxel Builder switched to it. Browser pane: a synthetic stretch grab starts with no jump, grows,
